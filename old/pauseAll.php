...
$events = $scheduler->getEventType('pause');
...
{
    $downloads = \SeedSync\Download::getOldStuff($db,$host->getHostId(),'DOWNLOADING');

    $numActive = count($downloads);

    echo date('d/m/Y H:i:s -')." Host {$host->getHost()} has $numActive active downloads".PHP_EOL;

    foreach($downloads as $download){
        echo 'Pausing download ' . $download->getId().PHP_EOL;
        echo shell_exec('/usr/bin/php '.__DIR__."/gui.php cli pauseDownload {$download->getId()}").PHP_EOL;
    }
    echo '------------------------------------'.PHP_EOL;
}